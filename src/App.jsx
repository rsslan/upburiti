    ...
    import { Upload, DollarSign, Percent, TrendingUp, CheckCircle, Home, ListChecks, RefreshCw, AlertCircle, CalendarDays, FileDown, Ruler, PlusCircle, MinusCircle, ArrowDownUp, LandPlot, Menu, X } from 'lucide-react';
...
const CONFIG = {
    ...
    const initialData = {
        ...
        vendasData: { unidades: [], unidadesVendidas: 0, unidadesEstoque: 0, valorEstoque: 0, receitas: { recebido: 0, aReceber: 0 }, inadimplentes: [], unidadesPermuta: 0, vgvTotal: 0, variacaoEstoqueUnidades: 0, variacaoEstoqueValor: 0, variacaoEstoquePercent: 0, precoMedioVendaMes: 0, variacaoInadimplenciaValor: 0, variacaoInadimplenciaPercent: 0, valorTotalCorrigido: 0, saldoDevedorTotal: 0, contratos: [] },
    ...
    const useChartReady = () => {
        ...
        useEffect(() => { const timer = setTimeout(() => setIsReady(true), 500); return () => clearTimeout(timer); }, []);
        return isReady;
    };
    ...
    const Card = ({ title, value, icon, color, smallText, size = 'md', tooltip }) => {
        const isSmall = size === 'sm';
        const [showTooltip, setShowTooltip] = useState(false);
        return (
            <div className={`bg-white rounded-xl shadow-md flex-1 min-w-[160px] ${isSmall ? 'p-2' : 'p-4'}`}>
                <div className={`flex items-center justify-between ${isSmall ? 'space-x-1' : 'space-x-2'}`}>
                    ...
                    {tooltip && (
                        <div className="relative group">
                            <button 
                                className="w-5 h-5 bg-blue-500 text-white rounded-full text-xs font-bold flex items-center justify-center hover:bg-blue-600 cursor-help"
                            ...
                            {showTooltip && (
                                <div className="absolute z-50 right-0 mt-2 w-56 bg-gray-900 text-white text-xs rounded-lg p-3 shadow-lg border border-gray-700 pointer-events-none group-hover:pointer-events-auto">
                                    {tooltip}
                                    <div className="absolute bottom-full right-2 w-2 h-2 bg-gray-900 transform rotate-45 -mb-1"></div>
                                </div>
                            )}
                        </div>
                    )}
    ...
    const ResumoPage = ({ data }) => {
        ...
        // Para a Resumo page, contar apenas apartamentos para venda (excluir PERMUTA)
        const vendaResumo = React.useMemo(() => {
            const totalParaVenda = vendasData?.unidades?.filter(u => !(u.Posicao && u.Posicao.toUpperCase() === 'PERMUTA')).length || 0;
            const vendidas = vendasData?.unidades?.filter(u => u.Posicao && u.Posicao.toUpperCase() === 'VENDIDO').length || 0;
            return { total: totalParaVenda, vendidas: vendidas };
        }, [vendasData]);
        ...
        const valorEstoqueResumo = React.useMemo(() => {
            if (vendasData?.valorEstoque > 0) return vendasData.valorEstoque;
            if (!vendasData?.unidades) return 0;
            return vendasData.unidades.reduce((sum, apt) => {
                const pos = (apt.Posicao || '').toString().toUpperCase();
                if (pos === 'DISPONÍVEL') {
                    return sum + parseCurrency(apt.Total);
                }
                return sum;
            }, 0);
        }, [vendasData]);
        ...
        return (
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-6">
                    <Card title="Viabilidade" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(viabilidade)}`} icon={<DollarSign className="text-green-500" />} color="text-green-600" tooltip="Valor total orçado para o projeto conforme a viabilidade econômica" />
                    <Card title="Custo Incorrido" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(custoTotal)}`} icon={<TrendingUp className="text-blue-500" />} color="text-blue-600" tooltip="Total de custos pagos até a data de referência (Obra + Admin + Incorporação)" />
                    <Card title="Gasto no Mês" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(gastoMes)}`} icon={<DollarSign className="text-amber-500" />} color="text-amber-600" tooltip="Somatório de todos os gastos (pagamentos) realizados no mês de referência" />
                    <Card title="Unidades Vendidas" value={`${vendaResumo.vendidas} de ${vendaResumo.total}`} icon={<CheckCircle className="text-indigo-500" />} color="text-indigo-600" tooltip="Quantidade de unidades vendidas em relação ao total de unidades para venda do empreendimento (108 - sem permutas)" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card title="Custo de Obra" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(custoObra)}`} icon={<Ruler className="text-blue-500" />} color="text-blue-600" tooltip="Despesas diretas com a execução da obra (mão de obra, materiais, etc)" />
                    <Card title="Custo Administração" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(custoAdmin)}`} icon={<Ruler className="text-sky-500" />} color="text-sky-600" tooltip="Despesas administrativas do empreendimento" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card 
                        ...
                        color="text-sky-600"
                        tooltip="Análise de custo por metro quadrado: Meta (viabilidade), Gasto (pago até agora) e Comprometido (comprometido com fornecedores). Comparação entre área total e área de apartamentos"
                    />
                    <Card 
                        title="Orçamento R$/M²" 
                        value={
                        ...
                        tooltip="Custo de obra por metro quadrado: Meta (orçamento inicial), Gasto (valor pago) e Comprometido (valor comprometido com fornecedores)"
                    />
                </div>
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <Card 
                        ...
                        value={
                            <div className="space-y-1 text-sm">
                                ...
                                <div>Área Apartamentos: {new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(totalApartmentArea)} m²</div>
                                <div>Área Comum: {new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(summaryData.projectInfo.area - totalApartmentArea)} m²</div>
                                <div>Área Equivalente: {new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(totalApartmentArea / summaryData.projectInfo.area)} </div>
                            </div>
                        }
                        icon={<Ruler className="text-blue-500" />} 
                        color="text-blue-600" 
                    />
                    <Card 
                        title="Viabilidade R$/M² Área Total" 
                        value={
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <div className="font-medium mb-1">Área Construída</div>
                    ...
                    />
                    <Card 
                        title="Viabilidade R$/M² Área Vendável" 
                        value={
                        <div className="space-y-1 text-sm">
                            <div className="font-medium mb-1">Área Vendável</div>
                                    <div><span className="text-gray-500">Área:</span> {new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(totalApartmentAreaMinusPermuta)} m²</div>
                                    <div><span className="text-gray-500">Valor Meta:</span> R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(summaryData.viabilidade / totalApartmentAreaMinusPermuta)}</div>
                                    <div><span className="text-gray-500">Valor Previsto:</span> R$ {new Intl.NumberFormat('pt-BR', formatOptions).format((summaryData.custoObra + summaryData.custoAdmin + summaryData.custoIncorporacao || 0) / totalApartmentAreaMinusPermuta)}</div>
                                    <div><span className="text-gray-500">Valor Comprometido:</span> R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(summaryData.comprometidoGlobal / totalApartmentAreaMinusPermuta)}</div>
                        </div>
                    }
                        icon={<Ruler className="text-indigo-500" />} 
                        color="text-indigo-600" 
                ...
                <div className="bg-white p-4 rounded-xl shadow-md">
                    <h3 className="font-bold text-gray-700 mb-4">Lucro Total do Empreendimento</h3>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <Card title="Valor Total Vendido (Corrigido)" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(valorTotalCorrigidoResumo)}`} icon={<CheckCircle className="text-green-500" />} color="text-green-600" />
                        ...
                        <Card title="VGV PREVISTO" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(valorTotalCorrigidoResumo + valorEstoqueResumo)}`} icon={<DollarSign className="text-blue-500" />} color="text-blue-600" />
                        <Card title="Custo Total Previsto" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(summaryData.custoObra + summaryData.custoAdmin + summaryData.custoIncorporacao || 0)}`} icon={<TrendingUp className="text-amber-500" />} color="text-amber-600" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                        <Card 
                            title="Lucro Liquido Previsto" 
                            value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(summaryData.lucroBruto || 0)}`} 
                            icon={<TrendingUp className={summaryData.lucroBruto >= 0 ? "text-green-500" : "text-red-500"} />} 
                            color={summaryData.lucroBruto >= 0 ? "text-green-600" : "text-red-600"}
                            tooltip="Lucro bruto estimado = (Vendido Corrigido + Estoque Simulado) - Custo Total Previsto"
                        />
                        <Card 
                            title="Recebido x Gasto (até o momento)" 
                            value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(summaryData.lucroLiquido || 0)}`} 
                            icon={<DollarSign className={summaryData.lucroLiquido >= 0 ? "text-green-500" : "text-red-500"} />} 
                            color={summaryData.lucroLiquido >= 0 ? "text-green-600" : "text-red-600"}
                            tooltip="Lucro líquido real até o momento = Total Recebido - Custo Total Incorrido (fluxo de caixa)"
                        />
                        <Card 
                            title="Margem de Lucro Prevista" 
                            value={`${(summaryData.lucroPercentual || 0).toFixed(2)}%`} 
                            icon={<Percent className={summaryData.lucroPercentual >= 0 ? "text-green-500" : "text-red-500"} />} 
                            color={summaryData.lucroPercentual >= 0 ? "text-green-600" : "text-red-600"}
                            tooltip="Percentual de retorno sobre o custo total previsto = (Lucro Bruto / Custo Total) * 100"
                        />
                    </div>
                </div>
                <div className="bg-white p-4 rounded-xl shadow-md">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <Card title="Preço Médio Vendido" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(avgPricePerM2Sold)}/m²`} icon={<DollarSign className="text-green-500" />} tooltip="Preço médio por m² de todas as unidades vendidas, conforme contratos" />
                        <Card title="Valor/m² Corrigido" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(vendasData.valorMedioM2Corrigido || 0)}/m²`} icon={<Ruler className="text-blue-500" />} color="text-blue-600" tooltip="Valor corrigido por inflação (INCC) do preço médio de venda"/>
                        <Card title="Incc m²" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format((vendasData.valorMedioM2Corrigido || 0) - avgPricePerM2Sold)}/m²`} icon={<Ruler className="text-blue-500" />} color="text-blue-600" smallText={`${(((vendasData.valorMedioM2Corrigido || 0) - (avgPricePerM2Sold)) / (avgPricePerM2Sold) * 100).toFixed(2)}%`} tooltip="Diferença e percentual do ajuste INCC sobre o preço de venda"/>
                        <Card title="Lucro m² (Meta)" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format((vendasData.valorMedioM2Corrigido || 0) - (summaryData.viabilidade / totalApartmentAreaMinusPermuta))}`} icon={<TrendingUp className="text-blue-500" />} color={(vendasData.valorMedioM2Corrigido || 0) - (summaryData.viabilidade / totalApartmentAreaMinusPermuta) >= 0 ? 'text-green-500' : 'text-red-500'} smallText={`${(((vendasData.valorMedioM2Corrigido || 0) - (summaryData.viabilidade / totalApartmentAreaMinusPermuta)) / (summaryData.viabilidade / totalApartmentAreaMinusPermuta) * 100).toFixed(2)}% do custo`} tooltip="Lucro por m² considerando o custo meta (viabilidade) = Preço Corrigido - Viabilidade/m²"/>
                        <Card title="Lucro m² Previsto" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format((vendasData.valorMedioM2Corrigido || 0) - ((summaryData.custoObra + summaryData.custoAdmin + summaryData.custoIncorporacao || 0) / totalApartmentAreaMinusPermuta))}`} icon={<DollarSign className="text-amber-500" />} color={(vendasData.valorMedioM2Corrigido || 0) - ((summaryData.custoObra + summaryData.custoAdmin + summaryData.custoIncorporacao || 0) / totalApartmentAreaMinusPermuta) >= 0 ? 'text-green-500' : 'text-red-500'} smallText={`${(((vendasData.valorMedioM2Corrigido || 0) - ((summaryData.custoObra + summaryData.custoAdmin + summaryData.custoIncorporacao || 0) / totalApartmentAreaMinusPermuta)) / ((summaryData.custoObra + summaryData.custoAdmin + summaryData.custoIncorporacao || 0) / totalApartmentAreaMinusPermuta) * 100).toFixed(2)}% do custo`} tooltip="Lucro por m² com custo previsto = Preço Corrigido - (Obra + Admin + Incorporação)/m²"/>
                        <Card title="Diferença Comprometido" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format((vendasData.valorMedioM2Corrigido || 0) - (summaryData.comprometidoGlobal / totalApartmentAreaMinusPermuta))}`} icon={<AlertCircle className="text-red-500" />} color={(vendasData.valorMedioM2Corrigido || 0) - (summaryData.comprometidoGlobal / totalApartmentAreaMinusPermuta) >= 0 ? 'text-green-500' : 'text-red-500'} smallText={`${(((vendasData.valorMedioM2Corrigido || 0) - (summaryData.comprometidoGlobal / totalApartmentAreaMinusPermuta)) / (summaryData.comprometidoGlobal / totalApartmentAreaMinusPermuta) * 100).toFixed(2)}% do custo`} tooltip="Diferença considerando valores já comprometidos com fornecedores = Preço Corrigido - Comprometido/m²"/>
                    </div>
                </div>                 
            </div>
        );
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white p-4 rounded-xl shadow-md">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <Card title="Delta Mensal (vs. Baseline)" value={`${deltaBaselineMensal >= 0 ? '+' : ''}${deltaBaselineMensal.toFixed(2)}%`} icon={<Percent className={deltaBaselineMensal >= 0 ? "text-green-500" : "text-red-500"} />} color={deltaBaselineMensal >= 0 ? "text-green-600" : "text-red-600"} smallText={deltaBaselineMensal >= 0 ? 'Adiantado no mês' : 'Atrasado no mês'} tooltip="Diferença entre o executado real e a linha de base prevista no mês (Realizado - Baseline)" />
                        <Card title="Delta Global (vs. Baseline)" value={`${deltaBaselineGlobal >= 0 ? '+' : ''}${deltaBaselineGlobal.toFixed(2)}%`} icon={<Percent className={deltaBaselineGlobal >= 0 ? "text-green-500" : "text-red-500"} />} color={deltaBaselineGlobal >= 0 ? "text-green-600" : "text-red-600"} smallText={deltaBaselineGlobal >= 0 ? 'Obra adiantada' : 'Obra atrasada'} tooltip="Diferença acumulada entre o executado real e a linha de base desde o início da obra" />
                        <Card title="Delta Mensal (vs. Projetado)" value={`${deltaProjetadoMensal >= 0 ? '+' : ''}${deltaProjetadoMensal.toFixed(2)}%`} icon={<Percent className={deltaProjetadoMensal >= 0 ? "text-green-500" : "text-red-500"} />} color={deltaProjetadoMensal >= 0 ? "text-green-600" : "text-red-600"} smallText={deltaProjetadoMensal >= 0 ? 'Acima do projetado' : 'Abaixo do projetado'} tooltip="Diferença entre o executado real e a projeção do mês (Realizado - Projetado)" />
                        <Card title="Delta Global (vs. Projetado)" value={`${deltaProjetadoGlobal >= 0 ? '+' : ''}${deltaProjetadoGlobal.toFixed(2)}%`} icon={<Percent className={deltaProjetadoGlobal >= 0 ? "text-green-500" : "text-red-500"} />} color={deltaProjetadoGlobal >= 0 ? "text-green-600" : "text-red-600"} smallText={deltaProjetadoGlobal >= 0 ? 'Obra adiantada' : 'Obra atrasada'} tooltip="Diferença acumulada entre o executado real e a projeção desde o início da obra" />
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-md">
                    ...
                    <div className="w-full h-[200px] min-w-[300px] flex items-center justify-center">
                        {isReady && <ResponsiveContainer width="100%" height={200} minWidth={300} minHeight={200}><BarChart data={evolucaoMesData} layout="vertical" margin={{ top: 20, right: 30, left: 20, bottom: 5 }} isAnimationActive={!isPrintMode}>
                ...
                <div className="bg-white p-4 rounded-xl shadow-md">
                    ...
                    <div className="w-full h-[450px] min-w-[600px] flex items-center justify-center">
                        {isReady && <ResponsiveContainer width="100%" height={450} minWidth={600} minHeight={450}>
                ...
                <div className="bg-white p-6 rounded-xl shadow-md">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <Card
                            ...
                            tooltip="Diferença entre Recebimento Total (mês) e Custo de Execução (Direto + Indireto). Mostra se o recebido é suficiente para cobrir custos de execução"
                        />
                        <Card
                            ...
                            tooltip="Diferença entre Recebimento Total (mês) e Gasto Total no Mês. Indicador de fluxo de caixa do período"
                        />
                        <Card
                            ...
                            tooltip="Diferença entre Recebimento Total (mês) e Custo Total Aplicado (Execução + Incorporação)"
    ...
    const MedCefPlsPage = ({ data }) => {
        ...
        const buildRows = useCallback((parentCod, level = 0) => {
            const rows = [];
            ...
            <div className="space-y-6">
                ...
                {chartData && chartData.length > 0 && (
                    <div className="bg-white p-4 rounded-xl shadow-md">
                        ...
                        <div className="w-full h-[500px] min-w-[400px] flex items-center justify-center">
        ...
        return (
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                    ...
                    <Card title="A Receber (Inadimp. Mês)" value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(vendasData.receitas.aReceber)}`} icon={<DollarSign className="text-amber-500" />} color='text-red-500' />
                </div>
                <div className="bg-white p-4 rounded-xl shadow-md">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        ...
                        <Card title="VGV do Empreendimento" value={<div className="space-y-1 text-sm"><div>VGV sem permutas: R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(adjustedVgv)}</div></div>} icon={<LandPlot className="text-indigo-500" />} />
                    </div>
                </div>
                <div className="bg-white p-4 rounded-xl shadow-md">
                ...
                <div className="bg-white p-4 rounded-xl shadow-md">
                    <h3 className="font-bold text-gray-700 mb-4">Análise de Contratos (Saldo Devedor)</h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <Card 
                            title="Valor Total Vendido (Corrigido INCC)" 
                            value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(vendasData.valorTotalCorrigido || 0)}`} 
                            icon={<DollarSign className="text-green-500" />} 
                            color="text-green-600"
                            smallText={`${(vendasData.contratos?.length || 0)} contratos`}
                        />
                        <Card 
                            title="Saldo a Receber" 
                            value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(vendasData.saldoDevedorTotal || 0)}`} 
                            icon={<AlertCircle className="text-green-500" />} 
                            color={vendasData.saldoDevedorTotal > 0 ? "text-green-600" : "text-green-600"}
                        />
                        <Card 
                            title="Valor/m² Corrigido" 
                            value={`R$ ${new Intl.NumberFormat('pt-BR', formatOptions).format(vendasData.valorMedioM2Corrigido || 0)}/m²`} 
                            icon={<Ruler className="text-blue-500" />} 
                            color="text-blue-600"
                        />
                    </div>
    ...
    const EstoqueSimuladorPage = ({ data }) => {
    const { vendasData } = data;

    const formatOptions = { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 };

    // 1. Função Única de Área
    const getArea = (unitId) => {
        const lastTwo = unitId % 100;
        if ([1, 2, 7, 8].includes(lastTwo)) return 52.2;
        if (lastTwo === 3) return 51.84;
    ...
    // 2. Cálculo da média real do m² do estoque
    const mediaM2EstoqueAtual = React.useMemo(() => {
        const disponiveis = vendasData.unidades.filter(
            u => (u.Posicao || '').toString().toUpperCase() === 'DISPONÍVEL'
        );
        if (disponiveis.length === 0) return 0;
        const somaValores = disponiveis.reduce((sum, u) => sum + parseCurrency(u.Total), 0);
        const somaAreas = disponiveis.reduce((sum, u) => sum + getArea(u[CONFIG.vendasEspelho.UNIDADE_ID]), 0);
        return somaAreas > 0 ? somaValores / somaAreas : 0;
    }, [vendasData.unidades]);

    // 3. Estados (Corretagem inicia em 0 e Preço inicia com a média corrigida)
    const [precoM2Simulado, setPrecoM2Simulado] = useState(
        formatarParaReal(mediaM2EstoqueAtual.toFixed(2).replace(/\D/g, ""))
    );
    const [percentualCorretagem, setPercentualCorretagem] = useState(0);

    useEffect(() => {
        if (mediaM2EstoqueAtual > 0) {
            setPrecoM2Simulado(formatarParaReal(mediaM2EstoqueAtual.toFixed(2).replace(/\D/g, "")));
        }
    }, [mediaM2EstoqueAtual]);

    // 4. Lógica da Simulação
    const simulacao = React.useMemo(() => {
        const precoM2Digitado = typeof precoM2Simulado === 'string' 
            ? (parseFloat(precoM2Simulado.replace(/\D/g, "")) / 100) || 0 
            : (parseFloat(precoM2Simulado) || 0);

        const corretagemPercent = parseFloat(percentualCorretagem) || 0;
        let totalTabelaBruto = 0, totalVgvSimulado = 0, totalLiquidoSimulado = 0;

        const disponiveis = vendasData.unidades.filter(u => (u.Posicao || '').toString().toUpperCase() === 'DISPONÍVEL');

        const unidadesProcessadas = disponiveis.map(unit => {
            const unitId = unit[CONFIG.vendasEspelho.UNIDADE_ID];
            const area = getArea(unitId);
            const valorTabelaOriginal = parseCurrency(unit.Total);
            const valorBrutoSimulado = area * precoM2Digitado;
            const valorLiquido = valorBrutoSimulado * (1 - corretagemPercent / 100);

            totalTabelaBruto += valorTabelaOriginal;
            totalVgvSimulado += valorBrutoSimulado;
            totalLiquidoSimulado += valorLiquido;

            return {
                unidade: unitId,
                area,
                valorTabela: valorTabelaOriginal,
                precoM2Base: area > 0 ? valorTabelaOriginal / area : 0,
                precoM2Simulado: precoM2Digitado,
                valorBrutoSimulado,
                corretagem: valorBrutoSimulado * (corretagemPercent / 100),
                valorLiquido
            };
        });

        const custoTotal = (data.summaryData.custoObra + data.summaryData.custoAdmin + data.summaryData.custoIncorporacao) || 0;

        return {
            unidades: unidadesProcessadas,
            totalTabelaBruto,
            totalVgvSimulado,
            totalLiquidoSimulado,
            custoTotal,
            lucroBrutoSimulado: (data.vendasData.valorTotalCorrigido + totalLiquidoSimulado) - custoTotal,
            valorVendidoAtualizado: data.vendasData.valorTotalCorrigido
        };
    }, [precoM2Simulado, percentualCorretagem, vendasData.unidades, data]);

    const andaresEstoque = React.useMemo(() => {
        const grouped = {};
        vendasData.unidades.forEach(unit => {
            const rawId = unit[CONFIG.vendasEspelho.UNIDADE_ID];
            const unitId = typeof rawId === 'number' ? rawId : parseInt(rawId, 10);
            const floor = isNaN(unitId) ? 0 : Math.floor(unitId / 100);
            if (!grouped[floor]) grouped[floor] = [];
            grouped[floor].push(unit);
        });
        Object.keys(grouped).forEach(floor => {
            grouped[floor].sort((a, b) => {
                const aId = parseInt(a[CONFIG.vendasEspelho.UNIDADE_ID], 10) || 0;
                const bId = parseInt(b[CONFIG.vendasEspelho.UNIDADE_ID], 10) || 0;
                return aId - bId;
            });
        });
        return grouped;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl shadow-md">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Preço de Venda Simulado (R$/m²)</label>
                        <input type="text" value={precoM2Simulado} onChange={(e) => setPrecoM2Simulado(formatarParaReal(e.target.value))} className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Corretagem (%)</label>
                        <input type="number" value={percentualCorretagem} onChange={(e) => setPercentualCorretagem(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
                    </div>
                    <Card title="Vendido Atualizado" value={`R$ ${new Intl.NumberFormat('pt-BR').format(simulacao.valorVendidoAtualizado)}`} size="sm" />
                </div>
            </div>

            {/* CARDS DE RESULTADO */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <Card title="VGV Estoque (Tabela)" value={`R$ ${new Intl.NumberFormat('pt-BR').format(simulacao.totalTabelaBruto)}`} />
                <Card title="VGV Estoque (Simulado)" value={`R$ ${new Intl.NumberFormat('pt-BR').format(simulacao.totalVgvSimulado)}`} color="text-blue-600" />
                <Card title="Estoque Líquido" value={`R$ ${new Intl.NumberFormat('pt-BR').format(simulacao.totalLiquidoSimulado)}`} color="text-green-600" />
                <Card title="Lucro Total Previsto" value={`R$ ${new Intl.NumberFormat('pt-BR').format(simulacao.lucroBrutoSimulado)}`} color="text-indigo-600" />
            </div>

            {/* TABELA DETALHADA */}
            <div className="bg-white p-4 rounded-xl shadow-md">
                <h3 className="font-bold text-gray-700 mb-4">Detalhamento de Unidades Disponíveis</h3>
                <div className="overflow-x-auto h-[40vh]">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-100 sticky top-0">
                            <tr>
                                <th className="px-4 py-3 text-left">Apto</th>
                                <th className="px-4 py-3 text-right">Área (m²)</th>
                                <th className="px-4 py-3 text-right bg-gray-200">Preço Tabela (Bruto)</th>
                                <th className="px-4 py-3 text-right">R$/m² Base</th>
                                <th className="px-4 py-3 text-right text-blue-700">R$/m² Simulado</th>
                                <th className="px-4 py-3 text-right">Total Simulado</th>
                                <th className="px-4 py-3 text-right text-red-600">Corretagem</th>
                                <th className="px-4 py-3 text-right text-green-700">Líquido Simulado</th>
                            </tr>
                        </thead>
                        <tbody>
                            {simulacao.unidades.map((unit, index) => (
                                <tr key={index} className="border-b hover:bg-gray-50 transition-colors">
                                    <td className="px-4 py-2 font-bold">{unit.unidade}</td>
                                    <td className="px-4 py-2 text-right">{unit.area.toFixed(2)}</td>
                                    <td className="px-4 py-2 text-right font-medium bg-gray-50">R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(unit.valorTabela)}</td>
                                    <td className="px-4 py-2 text-right text-gray-500">R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(unit.precoM2Base)}</td>
                                    <td className="px-4 py-2 text-right font-semibold text-blue-600">R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(unit.precoM2Simulado)}</td>
                                    <td className="px-4 py-2 text-right">R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(unit.valorBrutoSimulado)}</td>
                                    <td className="px-4 py-2 text-right text-red-500">- R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(unit.corretagem)}</td>
                                    <td className="px-4 py-2 text-right font-bold text-green-600">R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(unit.valorLiquido)}</td>
                                </tr>
                            ))}
                        </tbody>
                        <tfoot className="bg-gray-800 text-white font-bold sticky bottom-0">
                                <tr>
                                    <td className="px-4 py-3" colSpan="2">TOTAIS ({simulacao.unidades.length} unidades)</td>
                                    <td className="px-4 py-3 text-right">R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(simulacao.totalTabelaBruto)}</td>
                                    <td className="px-4 py-3"></td>
                                    <td className="px-4 py-3"></td>
                                    <td className="px-4 py-3 text-right">R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(simulacao.totalVgvSimulado)}</td>
                                    <td className="px-4 py-3 text-right text-red-300">- R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(simulacao.totalVgvSimulado * (percentualCorretagem/100))}</td>
                                    <td className="px-4 py-3 text-right text-green-400">R$ {new Intl.NumberFormat('pt-BR', formatOptions).format(simulacao.totalLiquidoSimulado)}</td>
                                </tr>
                            </tfoot>
                    </table>
                </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-md">
                <h3 className="font-bold text-gray-700 mb-4">Painel de Estoque</h3>
                <div className="space-y-2">
                    {Object.keys(andaresEstoque).sort((a, b) => b - a).map(andar => (
                        <div key={andar} className="grid grid-cols-10 gap-2 items-center">
                            <div className="col-span-1 text-center font-bold text-gray-600">{andar}º Andar</div>
                            <div className="col-span-9 grid grid-cols-8 gap-2">
                                {andaresEstoque[andar].map(unit => {
                                    const status = (unit.Posicao || '').toString().toUpperCase();
                                    const isDisponivel = status === 'DISPONÍVEL';
                                    const bgClass = isDisponivel ? 'bg-green-500' : 'bg-gray-200';
                                    const textClass = isDisponivel ? 'text-white' : 'text-black';
                                    const area = getArea(unit[CONFIG.vendasEspelho.UNIDADE_ID]);
                                    return (
                                        <div key={unit[CONFIG.vendasEspelho.UNIDADE_ID]} className={`p-1 rounded text-center text-xs font-bold ${bgClass} ${textClass}`}>
                                            <div className="leading-tight">{unit[CONFIG.vendasEspelho.UNIDADE_ID]}</div>
                                            <div className="text-[9px]">{area.toFixed(2)} m²</div>
                                        </div>
                                    );
                                })}
                            </div>
                ...
                <div className="mt-4 text-xs text-gray-500">Disponível = verde | Não disponível = cinza</div>
            </div>
        </div>
    );
};
    ...
    const parseCurrency = (value) => {
        ...
        // Remove R$ e espaços
        let cleanValue = String(value).replace(/[R$\s]/g, '');
        
        // Se a string tiver vírgula, trocamos para ponto para o parseFloat entender
    ...
    const processSaldoDevedorData = (data, espelhoData, config) => {
        if (!data || !data.length) return null;

        const c = config.saldoDevedor; // Atalho para o config de saldo devedor
        ...
        // Cálculo de área para o m² corrigido
        const getAreaLocal = (unitId) => {
            const lastTwo = unitId % 100;
            if ([1, 2, 7, 8].includes(lastTwo)) return 52.2;
            if (lastTwo === 3) return 51.84;
            if ([4, 6].includes(lastTwo)) return 50.85;
    ...
    export default function App() {
        ...
        const pages = {
            ...
            'Incorporação': <IncorporacaoPage data={dashboardData} />,
            ...
            'Estoque': <EstoqueSimuladorPage data={dashboardData} />,
        };
        ...
        useEffect(() => {
            const loadFilesAutomatically = async () => {
                try {
                    ...
                    const fileNames = ['financeiro.xlsx', 'evolucao.xlsx', 'vendas-espelho.xlsx', 'vendas-inadimplentes.xlsx', 'pls.xlsx', 'saldo-devedor.xlsx'];
                    const files = [];
                    
                    for (const fileName of fileNames) {
                        try {
                            // O "?t=" força o navegador a buscar a versão mais recente do disco
                            const response = await fetch(`${import.meta.env.BASE_URL}data/${fileName}?t=${timestamp}`);
                            if (response.ok) {
        ...
        const navItems = [
            ...
            { name: 'Incorporação', icon: <Home size={18} /> },
            ...
            { name: 'Estoque', icon: <Home size={18} /> },
        ];
        ...
        const processedSaldoDevedor = useMemo(() => {
            if (!rawFilesData?.saldoDevedor) return null;
            return processSaldoDevedorData(rawFilesData.saldoDevedor, rawFilesData?.vendasEspelho || null, CONFIG);
        }, [rawFilesData?.saldoDevedor, rawFilesData?.vendasEspelho]);
        useEffect(() => {
            if (!processedFinanceiro && !processedEvolucao && !processedVendas && !processedPls && !processedSaldoDevedor) {
            ...
            try {
                ...
                console.log("Arquivos Brutos Carregados:", rawFilesData); // Veja se o objeto saldoDevedor existe aqui
                ...
                if (processedSaldoDevedor) {
                    finalData.vendasData.valorTotalCorrigido = processedSaldoDevedor.valorTotalCorrigido;
                    finalData.vendasData.saldoDevedorTotal = processedSaldoDevedor.saldoDevedorTotal;
                    finalData.vendasData.contratos = processedSaldoDevedor.contratos;
                    finalData.vendasData.valorMedioM2Corrigido = processedSaldoDevedor.valorMedioM2Corrigido;
                }
                
                // Calcular lucro total do empreendimento
                                    // 1. Valor das Unidades Vendidas (Valor Atualizado dos Contratos)
                    const valorVendidoAtualizado = processedSaldoDevedor?.valorTotalCorrigido || 0;

                    // 2. Valor das Unidades a Vender (Preço de Estoque, que já exclui permutas no seu processamento)
                    const valorUnidadesAVender = finalData.vendasData.valorEstoque || 0;

                    // 3. Custo Total Incorrido até o momento (Custo Total do Financeiro)
                    const custoTotalIncorrido = finalData.summaryData.custoTotal || 0;

                    // LÓGICA CORRIGIDA: (Vendido Atualizado + Preço Unid. a Vender) - Custo Total
                    const lucroBrutoReal = (valorVendidoAtualizado + valorUnidadesAVender) - (finalData.summaryData.custoObra + finalData.summaryData.custoAdmin + finalData.summaryData.custoIncorporacao || 0);

                    // Margem de Lucro baseada no Custo
                    const lucroPercentualReal = custoTotalIncorrido > 0 ? (lucroBrutoReal / (finalData.summaryData.custoObra + finalData.summaryData.custoAdmin + finalData.summaryData.custoIncorporacao || 0)) * 100 : 0;

                    // Lucro Líquido (Dinheiro em caixa vs Gasto): Recebido Total - Custo Total
                    const totalRecebido = finalData.financeiroData.cards.recebimentoTotalAcumulado || 0;
                    const lucroLiquidoReal = totalRecebido - custoTotalIncorrido;

                    // Atualização dos campos no SummaryData
                    finalData.summaryData.lucroBruto = lucroBrutoReal;
                    finalData.summaryData.lucroLiquido = lucroLiquidoReal;
                    finalData.summaryData.lucroPercentual = lucroPercentualReal;
        ...
        }, [processedFinanceiro, processedEvolucao, processedVendas, processedPls, processedSaldoDevedor, rawFilesData]);
        return (
            <>
                <style>{`
                ...
                <div className={isPrintMode ? 'print-mode' : ''}>
                    <div id="print-area" className={!isPrintMode ? 'hidden' : 'block'}>
                        <div className="p-4">
                            ...
                            <div className="page-break"></div>
                            <section>
                                <h2 className="text-2xl font-bold text-gray-800 border-b-2 border-gray-400 pb-2 mb-6">Incorporação</h2>
                                <IncorporacaoPage data={dashboardData} />
                            </section>
                            <div className="page-break"></div>
                            <section>
                    ...
                    {!isPrintMode && (
                        <div className={`flex w-full ${isMobileMenuOpen ? 'h-screen overflow-hidden lg:h-auto lg:overflow-auto' : ''}`}>
                            <aside className={`w-56 bg-white shadow-lg p-4 flex-col flex print-hidden overflow-y-auto ${isMobileMenuOpen ? 'fixed top-0 left-0 h-full z-50' : 'hidden lg:flex lg:fixed lg:top-0 lg:left-0 lg:h-full lg:z-50'}`}>
                                ...
                                <nav className="flex-1"><ul>{navItems.map(item => (<li key={item.name}><button onClick={() => { setActivePage(item.name); setIsMobileMenuOpen(false); }} className={`w-full flex items-center space-x-3 p-3 my-1 rounded-lg text-sm font-medium ${activePage === item.name ? 'bg-blue-500 text-white shadow-md' : 'text-gray-600 hover:bg-gray-200'}`} >{item.icon}<span>{item.name}</span></button></li>))}</ul></nav>
                            ...
                            <main className={`flex-1 p-4 sm:p-6 lg:p-8 lg:ml-56 bg-gray-50 ${isMobileMenuOpen ? 'lg:pointer-events-auto pointer-events-none' : ''}`}>
                                <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                                    <div className="flex items-center space-x-4">
                                        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">
                                            {isMobileMenuOpen ? <X size={20} /> : <Menu size={20} />}
                                        </button>
                                        <h2 className="text-2xl font-bold text-gray-800">Painel de Resumo - UP Buriti</h2>
                                    </div>
